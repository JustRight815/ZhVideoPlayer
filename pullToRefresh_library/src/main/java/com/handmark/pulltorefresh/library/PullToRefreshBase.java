...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	protected LoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
		...
		//zh 设置刷新时头部的背景颜色
		layout.setBackgroundColor(getResources().getColor(R.color.black_10));
		return layout;
	}
	...
	protected void onPullToRefresh() {
		mHeaderLayout.getHeaderImage().setVisibility(View.VISIBLE);
		switch (mCurrentMode) {
			case PULL_FROM_END:
	...
	protected void onReset() {
		...
		mHeaderLayout.reset();
		mFooterLayout.reset();
		if(isRefreshComplete == true){
			isRefreshComplete = false;
			mHeaderLayout.getHeaderImage().setVisibility(View.INVISIBLE);
